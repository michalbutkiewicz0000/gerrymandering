...
| Interfejs WWW | `tests/test_frontend.py`, `tests/test_api.py` i `node --check frontend/app.js` | kreator krok-po-kroku dla laika (wybory → obszar na mapie → cel z listy → proste ustawienia → wynik w prostym języku), tryb ekspercki (JSON, reguły, certyfikat) schowany; lokalna mapa SVG bez CDN, spójne filtrowanie danych, alternatywy, eksport | automatyczne bramki wykonane; test E2E w przeglądarce (Playwright) potwierdził pełny przepływ na przykładzie; pozostaje ręczny przegląd ergonomii dużej migawki krajowej |
