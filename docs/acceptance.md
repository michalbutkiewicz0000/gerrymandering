...
| Jednostka podziału wg profilu | `tests/test_assembly.py`, `tests/test_elections.py`, `tests/test_graph.py` | `POST /api/districting/assemble`: Sejm/Senat/EP/sejmik → węzeł=powiat (głosy agregowane po TERYT), rada powiatu → gmina, rada gminy → obwód; rekonstrukcja tylko dla rady gminy | wykonane; na realnej migawce krajowej Sejm=380 powiatów/1003 krawędzie/0 błędów, sejmik woj. 14=42 powiaty, rada powiatu 1421=6 gmin |
...
| Interfejs WWW | `tests/test_frontend.py`, `tests/test_api.py` i `node --check frontend/app.js` | kreator krok-po-kroku dla laika (wybory → rodzaj wyborów i obszar → cel z listy → proste ustawienia → wynik w prostym języku); dla wyborów krajowych mapa powiatów całego kraju, dla samorządu kaskada województwo→powiat→gmina; tryb ekspercki (JSON, reguły, certyfikat) schowany; lokalna mapa SVG bez CDN, alternatywy, eksport | automatyczne bramki wykonane; serwerowe składanie zestawu (`/api/districting/assemble`) potwierdzone na realnej migawce; pozostaje ręczny przegląd ergonomii dużej migawki krajowej |
